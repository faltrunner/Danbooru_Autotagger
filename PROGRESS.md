...
- **問題**：右鍵圖片後，原生瀏覽器選單（另存圖片、複製圖片等）完全無法使用。
- **修正**：在 `initImageUploader()` 中追蹤連續兩次 `contextmenu` 事件的時間差與目標元素。若在 400ms 內對**同一張圖片**連按兩次右鍵，第二次不呼叫 `preventDefault()`，讓原生選單正常彈出。
- **行為**：
  - 單次右鍵 → 顯示「📤 Upload to Danbooru」自訂選單
  - 快速右鍵兩下 → 顯示瀏覽器原生右鍵選單（另存圖片等）
